...
v0.7.0 目标是在 HTTP Runner 闭环基础上提供移动端控制台和 Codex 参数配置：
...
7. 支持手机选择项目、Runner、模型、推理难度、sandbox，并提交受控目标任务。

v0.7.0 不解决多用户、多租户、复杂权限、自动提交、自动发布和无限自主迭代。
...
- 简单 HTML + 原生 JavaScript，不引入 Vue 或构建工具。
- API Token 保存在浏览器 localStorage。
- 支持查看 Runner、选择项目、选择 Runner、选择任务类型、模型、推理难度和 sandbox。
- 支持提交任务、查看最近任务、打开 log/result/diff、取消任务。
...
v0.7.0 支持按任务传入 Codex 参数：

- `sandbox` 默认 `workspace-write`，传给 `--sandbox`。
- `model` 非空且不是 `default` 时追加 `--model <model>`。
- `reasoning_effort` 非空且不是 `default` 时追加 `-c reasoning_effort=<value>`。
- 保留 `--output-last-message <result_file>`。
- 不添加当前 Codex CLI 未使用的额外参数。
